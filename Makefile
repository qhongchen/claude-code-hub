...
.PHONY: help dev-help app-update dev db app build build-nocache app-rebuild app-nocache prune-images rm-app-image compose clean migrate db-shell redis-shell logs logs-app logs-db logs-redis reset status stop
...
# root docker-compose.yaml 的 app-only 更新入口
app-update:
